...
pub fn render_thread_rss(
   ...
   let items: String = thread_tweets
      .iter()
      .filter(|tw| tw.available)
      .map(|tw| render_tweet_item(tw, url_prefix, config))
      .collect();
   rss_channel(
      &format!("{link}/rss"),
      &title,
...
fn render_tweet_items_with_pinned(
   ...
   {
      let link = tweet_link(pinned_tweet);
...
<br>Video<br>
  ...
  <source src="{url}" type="video/mp4"></video>"#
   ...
   if let Some(ref quote) = tweet.quote
      && quote.available
   {
      let quote_link = format!("{}{}", url_prefix, tweet_link(quote));
...
fn render_tweet_item(tweet: &Tweet, url_prefix: &str, config: &Config) -> String {
   ...
   let link = if tweet.retweet.is_some() && display_tweet.id != 0 {
      ...
      format!("{url_prefix}{}", tweet_link(display_tweet))
   };
